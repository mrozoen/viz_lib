...
was answered incorrectly, and makes one card per question:
    ...
    Front  the question stem and its options
...
def build_front(q):
    ...
    return f'<div class="stem">{stem}{tag}</div>{opts}'


def build_back(q):
    return "".join(
        f'<div>{letter}. {html.escape(clean(q["options"][letter]))}</div>'
...
def main():
    ...
    for path in sorted(args.answers):
        ...
        for q in questions:
            ...
            seen.add(key)
            cards.append((build_front(q), build_back(q),
    ...
    for front, back, source in cards:
        deck.add_note(genanki.Note(model=MODEL, fields=[front, back, source],
